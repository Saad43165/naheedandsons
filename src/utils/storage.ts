    ...
    .then(({ data, error }: { data: any; error: any }) => {
      if (!error && data) {
        const mapped: CompanySettings = {
          ...
          yearsExperience: data.years_experience ?? DEFAULT_SETTINGS.yearsExperience,
          projectsCompleted: data.projects_completed ?? DEFAULT_SETTINGS.projectsCompleted,
          clientSatisfaction: data.client_satisfaction ?? DEFAULT_SETTINGS.clientSatisfaction,
          completedOnTime: data.completed_on_time ?? DEFAULT_SETTINGS.completedOnTime,
